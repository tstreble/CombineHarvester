...
int main(int argc, char** argv) {
  ...
  std::string proc_fakes = "data_fakes";


  vector<string> bkg_procs_MC = {"TTW", "TTZ", "EWK", "Rares", "TTWW", "tHq", "tHW"};
  //vector<string> bkg_procs_MC = {"TTW", "TTZ", "EWK", "Rares", "TTWW"};
  ...
  cb.cp().process(ch::JoinStr({sig_procs, bkg_procs_MC}))
    .AddSyst(cb, "lumi_13TeV_2016", "lnN", SystMap<>::init(1.025));
  ...
  cb.cp().process({proc_fakes})
      .AddSyst(cb, "CMS_ttHl16_Clos_e_norm", "lnN", SystMap<>::init(0.95));
  cb.cp().process({proc_fakes})
      .AddSyst(cb, "CMS_ttHl16_Clos_m_norm", "lnN", SystMap<>::init(1.1));
 ...
 cb.cp().process(ch::JoinStr({sig_procs, bkg_procs_MC}))
      .AddSyst(cb, "CMS_ttHl16_trigger_3l", "lnN", SystMap<>::init(1.03));
 cb.cp().process(ch::JoinStr({sig_procs, bkg_procs_MC}))
